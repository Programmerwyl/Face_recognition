...
video_path = 'C:/Users/wyl/Desktop/data/retinaface/image/face.mp4'
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not ret:
            break
        bbox, points = model.detect_face(frame)
        for box, point in zip(bbox, points):
            ...
            aligned = model.get_input_single(frame,box[:4],point)
            embedding = model.get_feature(aligned)
            min_dist, identity = who_is_it(embedding,database)
            pt = (int(box[0]),int(box[1]) -20)
            cv2.putText(frame,identity,pt,cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 10, 100), 2)
